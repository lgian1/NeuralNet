...
class NeuralNet():
    def __init__(self,X,Y,sizeH=[2]):
        ...
        for i in range(self.lenght-1):
            self.layers[i].weights = [[rd.random() for j in range(self.layers[i+1].lenght)] for k in range(self.layers[i].lenght)] #MATRICE DI ADIACENZE RIGHE = LAYER 0, CLN = LAYER 1
        return 
